...
def patch_based_denoise(
    ...
    scale = torch.max(torch.norm(patches, dim=-1), dim=1, keepdim=True)[0]
    patches = patches / scale
    # out = model.sample(
    #     x_start=patches.transpose(1, 2), use_ema=cfg.use_ema, steps=cfg.steps, log_count=cfg.steps, verbose=True
    # )

    patches_denoised_list = []

    for i in range(patches.shape[0]):

        patch = patches[i:i+1].cuda()   # ⭐ 每次只搬一个

        # center & scale
        center = patch.mean(dim=1, keepdim=True)
        patch = patch - center
        scale = torch.max(torch.norm(patch, dim=-1))
        patch = patch / scale

        x_start = patch.transpose(1, 2)
        x_cond  = x_start
        ...
        patch_denoised = out["x_pred"].transpose(1, 2)

        patch_denoised = patch_denoised * scale + center

        patches_denoised_list.append(patch_denoised.cpu()) 

        del patch, x_start, x_cond, out 
