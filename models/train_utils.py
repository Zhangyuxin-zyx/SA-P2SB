...
def crop_patch_batch(x_gt, x_noisy, patch_size=2048):
    """
    On-the-fly patch cropping.
    x_gt, x_noisy: [B, 3, N]
    Returns: [B, 3, patch_size]
    """
    B, C, N = x_gt.shape
    if N <= patch_size:
        return x_gt, x_noisy
    ...
    # 转为 [B, N, 3] 计算距离
    x_gt_t = x_gt.transpose(1, 2)
    x_noisy_t = x_noisy.transpose(1, 2)

    # 1. 随机中心点
    seed_indices = torch.randint(0, N, (B, 1), device=x_gt.device)
    batch_indices = torch.arange(B, device=x_gt.device).view(-1, 1)
    seed_points = x_gt_t[batch_indices, seed_indices, :]

    # 2. 计算距离
    dists = torch.sum((x_gt_t - seed_points)**2, dim=-1) # [B, N]

    # 3. KNN 取 Patch
    _, patch_indices = torch.topk(dists, k=patch_size, largest=False) # [B, K]
    
    # 4. Gather
    idx_expanded = patch_indices.unsqueeze(-1).expand(-1, -1, 3)
    patch_gt = torch.gather(x_gt_t, 1, idx_expanded)
    patch_noisy = torch.gather(x_noisy_t, 1, idx_expanded)

    # 5. 归一化 (关键!)
    # 以 Noisy Patch 的重心为原点
    centroid = torch.mean(patch_noisy, dim=1, keepdim=True)
    patch_gt = patch_gt - centroid
    patch_noisy = patch_noisy - centroid
    
    # 缩放
    max_dist = torch.max(torch.sqrt(torch.sum(patch_noisy**2, dim=-1)), dim=1, keepdim=True)[0]
    scale = max_dist + 1e-6
    patch_gt = patch_gt / scale.unsqueeze(-1)
    patch_noisy = patch_noisy / scale.unsqueeze(-1)

    return patch_gt.transpose(1, 2).contiguous(), patch_noisy.transpose(1, 2).contiguous()