...
def load_diffusion(cfg: Dict) -> tuple:
    ...
    # 1. 获取 EdgeConv / Attention 产生的维度 (默认为 0)
    edge_conv_dim = cfg.diffusion.get("edge_conv_dim", 0)
    
    # 2. 获取基础条件 (x1) 的维度
    cond_x1 = cfg.diffusion.get("cond_x1", True)
    base_cond_dim = 3 if cond_x1 else 0
    
    # 3. 计算总共需要的额外通道数
    # Total = Base(3) + Geometric(0 or 32 or 64)
    required_extra_dim = base_cond_dim + edge_conv_dim
    
    logger.info(f"[Auto-Config] Dimensions Calculation: Base(x1)={base_cond_dim} + GeoModule={edge_conv_dim} => Total Extra={required_extra_dim}")

    # 4. 找到并强制修改 Backbone 配置
    target_cfg_node = None
    if "PVD" in cfg.model:
        target_cfg_node = cfg.model.PVD
    else:
        target_cfg_node = cfg.model

    if hasattr(target_cfg_node, "extra_feature_channels"):
        old_val = target_cfg_node.extra_feature_channels
        if old_val != required_extra_dim:
            logger.warning(f"[Auto-Config] Overwriting extra_feature_channels: {old_val} -> {required_extra_dim}")
            target_cfg_node.extra_feature_channels = required_extra_dim
    else:
        logger.info(f"[Auto-Config] Setting extra_feature_channels to {required_extra_dim}")
        target_cfg_node.extra_feature_channels = required_extra_dim

    # setup model
    backbone = load_model(cfg).to(cfg.local_rank)
    model = P2SB(cfg=cfg, model=backbone)
