...
from .EdgeConv import TimeGatedGeometryBlock

from .structural_loss import calc_selective_chamfer
...
class DualAttentionBlock(nn.Module):
    def __init__(self, in_channels, out_channels=64):
        ...
        self.fc_channel = nn.Sequential(
            ...
            nn.Conv1d(in_channels, in_channels * 2, 1), # 稍微升维提取特征
            nn.ReLU(),
    ...
    def forward(self, x):
        ...
        # Channel Attn
        w_ch = self.fc_channel(x) # [B, 3, 1]
        x_ch = x * w_ch
        
        # Spatial Attn
        max_pool = torch.max(x_ch, dim=1, keepdim=True)[0]
        avg_pool = torch.mean(x_ch, dim=1, keepdim=True)
        spatial_in = torch.cat([max_pool, avg_pool], dim=1) # [B, 2, N]
        w_sp = self.conv_spatial(spatial_in) # [B, 1, N]
        
        x_out = x_ch * w_sp
        
        return self.proj(x_out) # [B, 64, N]


# heavily adapted from https://github.com/NVlabs/I2SB
class P2SB(DiffusionModel):
    def __init__(self, cfg: Dict, model: torch.nn.Module):
        ...
        # 初始化 EdgeConv (可裁剪)
        # 默认值为 0，表示不启用
        self.edge_dim = cfg.diffusion.get("edge_conv_dim", 0)
        self.edge_k = cfg.diffusion.get("edge_conv_k", 20)

        # 读取模块类型，默认为 edgeconv 以兼容旧配置
        # 可选值: "edgeconv", "attention"
        self.geo_type = cfg.diffusion.get("geo_type", "edgeconv") 

        self.geo_encoder = None
        
        # if self.edge_dim > 0:
        #     print("self.edge_dim > 0")

        #     logger.info(f"[GeoSB] Dynamic EdgeConv Enabled: dim={self.edge_dim}, k={self.edge_k}")

        #     self.geo_encoder = EdgeConvBlock(in_channels=3, out_channels=self.edge_dim, k=self.edge_k).to(device)
        # else:
        #     encoder = None

        if self.edge_dim > 0:
            self.geo_encoder = TimeGatedGeometryBlock(3, self.edge_dim, k=20).to(device)
            # if self.geo_type == "attention":
            #     logger.info(f"[GeoSB] Using Dual-Attention Block (dim={self.edge_dim})")
            #     self.geo_encoder = DualAttentionBlock(in_channels=3, out_channels=self.edge_dim).to(device)
            # else:
            #     logger.info(f"[GeoSB] Using EdgeConv Block (dim={self.edge_dim}, k={self.edge_k})")
            #     self.geo_encoder = EdgeConvBlock(in_channels=3, out_channels=self.edge_dim, k=self.edge_k).to(device)
        else:
            logger.info("[GeoSB] Geometric Module Disabled.")
        ...
        self.calculate_loss = get_loss(cfg)
    ...
    def guided_step(self, xt: Tensor, source: Tensor) -> Tensor:
        """
        采样引导 (Guidance)
        在反向扩散过程中，利用梯度将 xt 拉向 source (观测值 x1) 的流形。
        """
        # 读取参数
        eta = self.cfg.sampling.get("guide_eta", 0.01)
        lam = self.cfg.sampling.get("guide_lambda", 0.96)
        ...
        with torch.enable_grad():
            ...
            # 计算 SCD Loss
            # calc_selective_chamfer 返回 [B]，我们需要 scalar 进行 backward
            loss = calc_selective_chamfer(xt_in, source, lambda_ratio=lam).sum()
            
            # 计算梯度
            grad = torch.autograd.grad(loss, xt_in)[0]
        
        # 梯度归一化 (防止梯度爆炸)
        # grad: [B, 3, N] -> norm over [3, N]
        grad_norms = torch.norm(grad.reshape(grad.shape[0], -1), dim=1, keepdim=True) # [B, 1]
        grad_norms = grad_norms.view(-1, 1, 1) # [B, 1, 1]
        
        # 安全除法
        normalized_grad = grad / (grad_norms + 1e-8)
        
        # 更新 xt (梯度下降方向)
        xt_out = xt_in - eta * normalized_grad
        
        return xt_out.detach()

    def refine_coords(self, x_pred: Tensor, x_ref: Tensor) -> Tensor:
        """
        测试时精修 (Test-Time Refinement)
        生成结束后，进一步微调坐标，使其贴合观测值 x1。
        """
        # 读取参数
        steps = self.cfg.sampling.get("refine_steps", 10)
        eta = self.cfg.sampling.get("refine_eta", 0.005)
        lam = self.cfg.sampling.get("refine_lambda", 0.96) # 建议比训练时稍低，只匹配最确定的部分
        
        with torch.enable_grad():
            # 复制一份用于优化
            x_opt = x_pred.clone().detach().requires_grad_(True)
            optimizer = torch.optim.SGD([x_opt], lr=eta)
            
            # 简单的优化循环
            # 注意：这里不开 tqdm 避免刷屏，或者可以加 verbose 控制
            for _ in range(steps):
                optimizer.zero_grad()
                # 计算 Loss
                loss = calc_selective_chamfer(x_opt, x_ref, lambda_ratio=lam).sum()
                loss.backward()
                optimizer.step()
    ...
    def sample_ddpm(
        ...
        # [检查开关] 是否启用引导
        enable_guidance = self.cfg.sampling.get("enable_guidance", False)
        
        for prev_step, step in pair_steps:
            assert prev_step < step, f"{prev_step=}, {step=}"

            if enable_guidance:
                # 使用 x_cond 作为引导源，如果 x_cond 为空则用 x1
                # 通常去噪任务 x_cond 就是 x1 (Noisy Input)
                guide_source = x_cond if x_cond is not None else x1
                # 确保维度匹配 [B, 3, N]
                if guide_source.shape[1] != 3 and guide_source.shape[2] == 3:
                     guide_source = guide_source.transpose(1, 2)
                # 执行一步梯度引导
                xt = self.guided_step(xt, guide_source)
    ...
    def ddpm_sampling(
        ...
        def pred_x0_fn(xt, step, x1, x_cond=None):
            ...
            # 1. 基础条件处理
            if self.cond_x1:
            # [关键修复] 使用 is not None 判断 Tensor
                if x_cond is not None:
                    if x_cond is x1:
                        base_cond = x1
                    else:
                        base_cond = torch.cat([x1, x_cond], dim=1)
                else:
                    base_cond = x1
            else:
                base_cond = x_cond
            ...
            if hasattr(self, 'geo_encoder') and self.geo_encoder is not None:
                geo_feat = self.geo_encoder(xt)
                if base_cond is not None:
                    x_cond = torch.cat([base_cond, geo_feat], dim=1)
                else:
                    x_cond = geo_feat
            else:
                x_cond = base_cond
    ...
    def sample(
        ...
        if self.cfg.diffusion.sampling_strategy == "DDPM":
            ...
            # 默认不精修，除非 config 开启
            x_refined = None
            if self.cfg.sampling.get("enable_refine", False):
                if verbose: 
                    logger.info("[GeoSB] Running Test-Time Refinement...")
                
                # 确定参考源 (x_start 是含噪输入)
                ref_source = x_start
                
                # 运行精修
                # 注意：这里需要临时开启 grad，因为 sample 函数有 @torch.no_grad() 装饰器
                with torch.enable_grad():
                     x_refined = self.refine_coords(x_pred, ref_source)

            data = {
                "x_chain": xs,
                "x_pred": x_refined if x_refined is not None else x_pred, # 如果精修了，主结果返回精修后的
                "x_pred_raw": x_pred, # 保留原始结果以备对比
                "x_start": x_start,
            }
    ...
    def forward(
        ...
        # 将 noise_levels 的定义移到最最前面！
        # 必须在调用 geo_encoder 之前定义它
        ...
        # 3. 几何特征注入 (GeoSB 核心逻辑)
        if hasattr(self, 'geo_encoder') and self.geo_encoder is not None:
            # 这里的参数名必须是 noise_levels，绝对不能是 t
            geo_feat = self.geo_encoder(xt, noise_levels)
            
            if base_cond is not None:
                x_cond_final = torch.cat([base_cond, geo_feat], dim=1)
            else:
                x_cond_final = geo_feat
        else:
            x_cond_final = base_cond

        # 4. 模型预测
        pred = self.model(xt, noise_levels, x_cond=x_cond_final)
