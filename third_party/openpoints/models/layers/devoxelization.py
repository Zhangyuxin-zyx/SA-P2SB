...
class TrilinearDevoxelization(Function):
    ...
    def forward(ctx, features, coords, resolution, is_training=True):
        ...
        outs, inds, wgts = pointnet2_cuda.trilinear_devoxelize_forward(resolution, is_training, coords, features)
        if is_training:
    ...
    def backward(ctx, grad_output):
        ...
        grad_inputs = pointnet2_cuda.trilinear_devoxelize_backward(grad_output.contiguous(), inds, wgts, ctx.r)
        return (
            grad_inputs.view(grad_output.size(0), grad_output.size(1), ctx.r, ctx.r, ctx.r),
            None,
            None,
            None,
        )
