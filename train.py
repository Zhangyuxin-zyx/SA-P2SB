...
from models.train_utils import get_data_batch, getGradNorm, set_seed, setup_output_subdirs, to_cuda, crop_patch_batch
...
def train(cfg: DictConfig) -> None:
    ...
    for step in range(cfg.start_step, cfg.training.steps):
        ...
        for accum_iter in range(cfg.training.accumulation_steps):
            ...
            # >>>>>>>>>>>> 新增：Patch Training 开关逻辑 <<<<<<<<<<<<
            # 从配置读取开关和大小
            use_patch = cfg.training.get("use_patch", False)
            patch_size = cfg.training.get("patch_size", 2048)

            if use_patch:
                # 执行切片
                x_gt, x_start = crop_patch_batch(x_gt, x_start, patch_size=patch_size)
                # 更新 x_cond (通常去噪任务 x_cond 就是输入噪声)
                # x_cond = x_start 
                x_cond = None
            # >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
